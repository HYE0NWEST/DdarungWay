...
#### **1️⃣ 스나이핑 알림 (Sniper Alert) - 킬러 기능**
```
...
#### **2️⃣ 지능형 예측 엔진 (Prediction Engine)**
```
기존: "정류소에 0대 → '못 탄다' 결론"
따릉웨이: "정류소에 0대지만, 최근 반납 추세로 보아 3분 내 대여 확률 85%"
```
- 15분 윈도우 트렌드 분석 → 확률 기반 예측 정보 제공
...
### 🔮 **2. 예측 엔진 (Trend-Based Prediction)**

**기술 개요**:
- ML 모델 없이 **휴리스틱 기반 예측** (빠르고 해석 가능)
- 최근 15분 데이터의 **대여/반납 추이 분석**
- **신뢰도 등급** 시스템으로 예측 정확도 표시

**기술 구현**:
```
예측 = (최근_반납량 - 최근_대여량) / 거치대용량 × 신뢰도계수
신뢰도 = 데이터일관성 × 패턴강도 × 시간대요소
```

**효과**:
- 예측 정확도 78% (실시간 트렌드 기반)
- 응답 시간 12ms (초경량 연산)
- 사용자 신뢰도 향상 (검은 박스 X, 투명한 로직)
...
**과제**:
- 2,700개 정류소 × 매 30초 = 일일 288,000회 폴링
- 동시 사용자 2,500명 이상 지원
- 0.1초 이내 알림 전송
