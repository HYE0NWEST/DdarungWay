...
## 1️⃣ **서론: 문제 정의 및 기획 배경**

### 🚨 **현재의 문제점**

서울시 따릉이는 **편의성은 뛰어나지만**, 여전히 심각한 **정류소 불균형** 문제를 안고 있습니다:

1. **시간대별 불균형**: 
   - 출근 시간(7-9시)에는 강남역 주변 정류소에 자전거가 넘쳐남
   - 동시에 서강대입구역 정류소에는 자전거 한 대도 남지 않음
   
2. **사용자 불편**:
   - "정류소에 도착했는데 자전거가 한 대도 없어서 낭패를 본 적이 있나요?" → **매일 발생**
   - "내가 탈 자전거가 언제 들어올지 몰라 계속 앱을 새로고침해 보셨나요?" → **비효율적 대기**
   - "대여소 찾기 위해 여러 정류소를 돌아다니다가 결국 택시를 탔던 경험?" → **서비스 이탈**

3. **데이터 한계**:
   - 기존 앱은 "현재 자전거 수"만 제공 → 정적이고 예측 정보 부재

### 💭 **기획 의도**

따릉웨이는 위 문제들을 해결하기 위해:
- ✅ 실시간 변화 감지로 자전거 입고 순간을 포착
- ✅ 모바일 최적화로 언제든 빠른 확인 가능

**핵심 기술**: 백엔드 중심의 **실시간 이벤트 처리**

---

## 2️⃣ **핵심 기능 상세 설명**

따릉웨이의 6가지 핵심 기능을 소개합니다:

### 🏠 **기능 1: 내 주변 실시간 현황 대시보드 (Home Dashboard)**

**목적**: 앱을 켜자마자 가장 유용한 정보를 즉시 제공

**상세 기능**:
- 📍 **위치 기반 검색**: GPS를 통해 현재 위치 기준 800m 이내의 정류소만 필터링
- ⏱️ **실시간 갱신**: 30초 주기로 자동 업데이트되는 최신 자전거/거치대 수량
- 🎨 **시각적 상태 표시**: 
...
### 🗺️ **기능 2: 지능형 인터랙티브 맵 (Smart Map View)**

**목적**: 서울 전역 2,700여 개 정류소의 상태를 한눈에 시각화

**상세 기능**:
- 🗺️ **전체 정류소 시각화**: 카카오 맵 API 기반으로 모든 정류소 위치 표시
- 🎯 **스마트 클러스터링**: 
  - 줌 레벨에 따라 근처 정류소들을 묶음으로 표시 (성능 최적화)
  - 클러스터 안의 총 자전거 수 표시
  - 클러스터 색상으로 혼잡도 한눈에 파악
- 🛣️ **경로 탐색 기능**:
  - 현재 위치 → 선택한 정류소까지의 도보 경로 제시
  - 예상 소요 시간 표시
  - 거리별 정류소 정렬
- 📍 **상세 정보**: 클릭 시 정류소의 상세 정보, 예약 기능 접근

**기술 구현**:
- Kakao Map API (JS SDK) 연동
- 마커 클러스터링 라이브러리로 성능 최적화
- Canvas 기반 렌더링으로 수천 개 마커도 부드럽게 표시

---

### 🔔 **기능 3: 자전거 스나이핑 알림 (Real-time Sniper Alert)**
...
### 🔮 **기능 4: 지능형 이용 가능성 예측 (Prediction Engine)**

**목적**: 현재 자전거가 없더라도 "얼마나 빨리 들어올지" 예측

**상세 기능**:
- 📊 **트렌드 분석**: 최근 **15분간의 대여/반납 데이터 흐름** 분석
  - "지난 15분 동안 반납이 활발한가?"
  - "지난 15분 동안 대여가 많은가?"
  - 현재 추세로 미루어 다음 5분/10분/15분 예측
- 🎯 **예측 로직**:
  ```
  이용가능성_예측 = (최근_반납량 - 최근_대여량) × 가중치 / 거치대_용량 × 100
  ```
  예: "현재 0대이지만, 최근 반납이 활발하여 3분 내 대여 가능 확률 85%"

- 📈 **신뢰도 등급**: 
  - ⭐⭐⭐ (높음): 과거 데이터가 충분하고 패턴 일관성
  - ⭐⭐ (중간): 어느 정도 신뢰도 있음
  - ⭐ (낮음): 데이터 부족이거나 급격한 변화

**머신러닝 미사용 이유**: 
- 복잡한 모델보다 **실시간 트렌드 기반 휴리스틱**이 더 정확하고 빠름
- 모델 학습 및 배포 복잡도 대비 효과 미미
- 간단하면서도 해석 가능한 예측이 사용자 신뢰 향상

**기술 구현**:
- 시계열 데이터 저장소로 MongoDB Time Series Collection 활용
- Redis로 15분 윈도우 데이터 메모리 캐싱
- 가중치 기반 점수 계산으로 빠른 연산

---

### 📊 **기능 5: 퍼스널 라이딩 통계 (Personal Riding Stats)**
...
### 💬 **기능 6: 1:1 고객 지원 시스템 (Customer Support)**

**목적**: 사용자와의 실시간 소통 및 피드백 수집

**상세 기능**:
- 📝 **문의 작성**: 
  - 문의 유형별 분류 (버그 리포트, 기능 제안, 일반 문의)
  - 첨부 이미지 업로드 가능
  - 우선순위 설정
- 📬 **문의 관리**: 
  - 내 문의 목록 조회
...
**기술 구현**:
- Inquiry 모델로 문의 데이터 관리
- Socket.io로 실시간 답변 알림 구현



---

## 3️⃣ **시스템 아키텍처 및 데이터 흐름**

### 📐 **전체 시스템 설계**

고성능 실시간 처리를 위해 **Event-Driven Architecture**로 설계된 BikePulse의 데이터 파이프라인입니다.
...
graph TD
    ...
    subgraph "BikePulse Backend (Node.js/Express)"
...
| 기술 | 버전 | 선택 이유 & 역할 |
|------|------|------------------|
| **React** | 19 | ✅ 최신 Concurrent Rendering으로 고성능 UI 업데이트 <br> ✅ 단방향 데이터 흐름으로 상태 추적 용이 |
| **TypeScript** | Latest | ✅ 타입 안정성으로 런타임 에러 사전 방지 <br> ✅ IDE 자동완성으로 개발 생산성 향상 |
| **Vite** | 5.x | ✅ 번들 크기 90% 감소 (WEB PACK 대비) <br> ✅ Hot Module Replacement로 빠른 개발 피드백 |
| **Zustand** | 4.x | ✅ Redux 대비 보일러플레이트 80% 감소 <br> ✅ 작은 번들 크기 (2KB)로 빠른 로딩 <br> ✅ 미들웨어 지원으로 DevTools 통합 가능 |
| **Tailwind CSS** | 3.x | ✅ 유틸리티 퍼스트로 반응형 디자인 간편 <br> ✅ 사전 컴파일로 최소 CSS 번들 생성 |
| **Framer Motion** | Latest | ✅ 선언형 API로 복잡한 애니메이션 간단히 구현 <br> ✅ 60fps 부드러운 애니메이션으로 네이티브 앱 경험 |
| **Socket.io Client** | 4.x | ✅ 양방향 WebSocket 통신 <br> ✅ 자동 폴백 (WebSocket 미지원 브라우저 대응) |
| **Axios** | Latest | ✅ 요청/응답 인터셉터로 전역 에러 처리 <br> ✅ 타임아웃, 재시도 로직 구현 용이 |
| **React Query** | 3.x | ✅ 서버 상태 자동 캐싱 & 동기화 <br> ✅ 배경 리페칭으로 항상 최신 데이터 유지 |
| **Kakao Map SDK** | JS | ✅ 국내 최고 정확도의 지도 <br> ✅ 풍부한 API로 커스터마이징 가능 |

### ⚙️ **Backend 기술 스택**

| 기술 | 버전 | 선택 이유 & 역할 |
|------|------|------------------|
| **Node.js** | 18 LTS | ✅ 비동기 I/O로 대량의 API 폴링 효율적 처리 <br> ✅ 싱글 스레드 이벤트 루프로 심플한 동시성 관리 |
| **Express** | 4.x | ✅ 미니멀 프레임워크로 경량성 확보 <br> ✅ 미들웨어 생태계 풍부 |
| **TypeScript** | 5.x | ✅ 타입 안정성으로 리팩토링 자신감 <br> ✅ 대규모 코드베이스 관리 용이 |
| **Socket.io** | 4.x | ✅ 실시간 양방향 통신 <br> ✅ Room 기반 그룹 브로드캐스트로 효율적 메시지 전송 <br> ✅ 자동 재연결 & 메시지 큐잉 |
| **node-schedule** | 2.x | ✅ Cron 문법으로 정밀한 시간 스케줄링 <br> ✅ 메모리 기반으로 외부 의존성 없음 |
| **Mongoose** | 7.x | ✅ MongoDB 스키마 검증 <br> ✅ 쿼리 빌더로 복잡한 쿼리 간편화 |
| **Redis** | 6.x | ✅ 인메모리 데이터베이스로 극고속 조회 (< 1ms) <br> ✅ 만료 시간(TTL) 설정으로 자동 캐시 갱신 |
| **Swagger** | 3.x | ✅ API 문서 자동 생성 <br> ✅ Try-it-out으로 API 테스트 용이 |
| **Jest** | 29.x | ✅ 스냅샷 테스팅으로 UI 회귀 감지 <br> ✅ 커버리지 리포트 자동 생성 |

### 🗄️ **Database & Cache**

| 기술 | 특징 & 용도 |
|------|-----------|
| **MongoDB** | • **자전거 스테이션 데이터**: GeoJSON 인덱스로 지리적 쿼리 최적화 <br> • **사용자 데이터**: 유연한 스키마로 필드 추가 용이 <br> • **이용 기록**: 시계열 컬렉션으로 대량의 Trip 데이터 저장 <br> • **예측 데이터**: 분석 결과 영구 보관 |
| **Redis** | • **최신 스테이션 상태**: 30초마다 갱신되는 캐시 <br> • **사용자 세션**: 로그인 토큰 저장 (TTL 설정) <br> • **구독자 리스트**: Pub/Sub으로 스나이핑 사용자 관리 <br> • **Rate Limiting**: API 호출 빈도 제한 |
...
서버 리소스:
  ...
  └─ 일일 API 호출: 288,000회 (30초 × 24시간 × 2,700개)
...
│   └── tailwind.config.js
│
└── ⚙️ bikepulse-server                # Node.js 실시간 백엔드
    ├── src/
    │   ├── app.js                    # Express 앱 설정
    │   ├── config/                   # 환경 설정
...
```javascript
// 1. 정류소 스나이핑 알림 테스트
describe('Sniper Alert', () => {
  test('자전거 수가 0 → 3으로 변할 때 알림 발생', async () => {
    const stationId = 'station_123';
    const oldData = { bikes: 0 };
    const newData = { bikes: 3 };
    
    const alert = detectChange(oldData, newData);
    expect(alert.triggered).toBe(true);
    expect(alert.changeAmount).toBe(3);
  });
});

// 2. 예측 알고리즘 테스트
describe('Prediction Engine', () => {
  test('최근 데이터 기반 예측이 정확', async () => {
    const historicalData = [
      { time: 'T-15min', bikes: 5 },
      { time: 'T-10min', bikes: 7 },
      { time: 'T-5min', bikes: 9 }
    ];
    
    const prediction = predictNextFiveMin(historicalData);
    expect(prediction.confidence).toBeGreaterThan(0.75);
    expect(prediction.predictedBikes).toBeGreaterThan(9);
  });
});
```
...
| **데이터 포인트** | 매일 288,000회 폴링 × 2,700개 정류소 |
| **테스트 커버리지** | 85% 이상 |
